...
icu$thirty_day_mort_lab <- factor(icu$thirty_day_mort_lab, labels = c("Did not die", "Died within 30 days"))
ui <- fluidPage(
  ...
  sidebarPanel(
    selectInput("varlab", 
                ...
                choices =  c("Creatinine", 
                             ...
                             "Calcium"),
                selected = "Creatinine"),
    checkboxInput("dead", "Seperate those who did and did not die within 30 days of admission", 
                  value = FALSE),
    ...
    sidebarPanel(
      ...
      selectInput("varvit", 
                  ...
                  choices =  c("Heart Rate", 
                               ...
                               "Respiratory Rate"),
                  selected = "Heart"),
      checkboxInput("dead2", "Seperate those who did and did not die within 30 days of admission", 
                    value = FALSE),
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("dem", 
                  label = "Choose a demographic to view 30-day mortality summary",
                  choices = c("Ethnicity", "Language", "Insurance Type", 
                              "Marital Status", "Gender", 
                              "Age at Hospital Admission", "First ICU Care Unit"),
                  selected = "Ethnicity")), 
    mainPanel(
      plotOutput("demoPlot"), 
      tableOutput("demoTab")
...
output$labeventsPlot <- renderPlot({
  ...
  else if (input$dead== TRUE & input$outlier == FALSE) {ggplot(data = icu, aes(x = data_in)) + geom_histogram() + theme_minimal()+
      ggtitle(paste("Distribution of", input$varlab))+xlab(input$varlab) +
      facet_grid(. ~ thirty_day_mort_lab)}
  else if (input$dead== FALSE & input$outlier == TRUE) {ggplot(data = icu, aes(x = data_in)) + geom_histogram() + theme_minimal()+
      ggtitle(paste("Distribution of", input$varlab))+xlab(input$varlab) +
      xlim(c((summary(data_in)[2]-1.5*(summary(data_in)[5]-summary(data_in)[2])), (summary(data_in)[5]+1.5*(summary(data_in)[5]-summary(data_in)[2]))))}
  else {ggplot(data = icu, aes(x = data_in)) + geom_histogram() + theme_minimal()+
      ggtitle(paste("Distribution of", input$varlab))+xlab(input$varlab) +
      xlim(c((summary(data_in)[2]-1.5*(summary(data_in)[5]-summary(data_in)[2])), (summary(data_in)[5]+1.5*(summary(data_in)[5]-summary(data_in)[2])))) +
      facet_grid(. ~ thirty_day_mort_lab)}
  })
...
output$vitalsPlot <- renderPlot({
  ...
  else if (input$dead2 == TRUE & input$outlier2 == FALSE) {ggplot(data = icu, aes(x = data_vit)) + geom_histogram() + theme_minimal()+
      ggtitle(paste("Distribution of", input$varvit))+xlab(input$varvit) +
      facet_grid(. ~ thirty_day_mort_lab)}
  else if (input$dead2 == FALSE & input$outlier2 == TRUE) {ggplot(data = icu, aes(x = data_vit)) + geom_histogram() + theme_minimal()+
      ggtitle(paste("Distribution of", input$varvit))+xlab(input$varvit) +
      xlim(c((summary(data_vit)[2]-1.5*(summary(data_vit)[5]-summary(data_vit)[2])), (summary(data_vit)[5]+1.5*(summary(data_vit)[5]-summary(data_vit)[2]))))}
  else {ggplot(data = icu, aes(x = data_vit)) + geom_histogram() + theme_minimal()+
      ggtitle(paste("Distribution of", input$varvit))+xlab(input$varvit) +
      xlim(c((summary(data_vit)[2]-1.5*(summary(data_vit)[5]-summary(data_vit)[2])), (summary(data_vit)[5]+1.5*(summary(data_vit)[5]-summary(data_vit)[2])))) +
      facet_grid(. ~ thirty_day_mort_lab)}
})
...
output$demoPlot <- renderPlot({
  ...
  ggplot(data = icu, aes(x= data_dem, y = icu$thirty_day_mort)) + geom_col() +
    theme_minimal()+ ggtitle(paste("Distribution of", input$dem))+xlab(input$dem) +
