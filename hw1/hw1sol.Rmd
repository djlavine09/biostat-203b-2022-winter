...
``` 
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $1}' | sort | uniq| awk 'END {print NR}'
```
There are 523741 total rows in this data file, and there are 356879 unique patients. 
...
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz |  awk -F, '{print $6}' |sort |uniq
``` 
Print the number of unique values of `admission_type`
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz |  awk -F, '{print $6}' |sort |uniq | awk 'END {print NR}'
```


Show the possible values of `admission_location`
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz |  awk -F, '{print $7}' |sort |uniq
``` 
Print the number of unique values of `admission_location`
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz |  awk -F, '{print $7}' |sort |uniq | awk 'END {print NR}'
```


Show the possible values of `insurance`
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz |  awk -F, '{print $9}' |sort |uniq
``` 
Print the number of unique values of `insurance`
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz |  awk -F, '{print $9}' |sort |uniq | awk 'END {print NR}'
```
Show the possible values of `ethnicity`
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz |  awk -F, '{print $12}' |sort |uniq
``` 
Print the number of unique values of `admission_type`
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz |  awk -F, '{print $12}' |sort |uniq | awk 'END {print NR}'
```
