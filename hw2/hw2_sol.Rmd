...
```{r}
labevents_tble %>% group_by(itemid) %>% summarise(mean = mean(na.omit(valuenum)),
...
mimic_icu_cohort$thirty_day_mort <- 
  as.numeric((ymd_hms(mimic_icu_cohort$deathtime) - ymd_hms(mimic_icu_cohort$admittime)) <= 30*60*24)
```
